...
def average_single_label(tabels, dataset):
   ...
   # titel = 'Deep_Rule_Set similarity \n   label predicted from NN for train data \n'+ dataset
    ...
    fig, ax = plt.subplots()
    graph_with_error_bar(x_values, y_values, y_stdr, fix_y_axis=True , y_axis_tile='accuracy [%]', ax_out=ax)
...
def graph_with_error_bar(x_values, y_values, y_stdr, title = "",x_axis_title="", y_axis_tile='', fix_y_axis= False, ax_out = False ):
    ...
    if not ax_out:
        ...
        plt.savefig('/home/jbrugger/IdeaProjects/DCDL_smaller/comparison_DCDL_vs_SLS/results/accuracy_dither_methods.png',
                dpi=300)
