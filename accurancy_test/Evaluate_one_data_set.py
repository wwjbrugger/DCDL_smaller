...
def average_accurancy_on_test_data(tabels, dataset):
    ...
    x_values = ['Deep_Rule_Set', 'SLS_Black_\nBox_Prediction', 'SLS_Black_\nBox_Label', 'Neural Network']
    ...
    graph_with_error_bar(x_values, y_values, y_stdr, titel, fix_y_axis=True, x_axis_title="method",
                         y_axis_tile='similarity [%]')
...
def graph_with_error_bar(x_values, y_values, y_stdr, title,x_axis_title="", y_axis_tile='', fix_y_axis= False ):
    fig, ax = plt.subplots()
...
if __name__ == '__main__':
    ...
    y_values = []
    y_stdr = []
    for path_to_results in ['data/numbers/results', 'data/fashion/results', 'data/cifar/results']:
        ...
        average_accurancy_on_test_data(table,path_to_results[5:-8])
